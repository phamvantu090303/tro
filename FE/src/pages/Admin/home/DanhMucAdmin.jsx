...
export default function DanhMucAdmin() {
    ...
    try {
      if (modal === "edit") {
        await axiosInstance.post(
          `/danh-muc/update/${selectedAdmin._id}`,
          selectedAdmin
        );
        alert("Updated successfully!");
      } else {
        await axiosInstance.post("/danh-muc/create", selectedAdmin);
  ...
  const handleDeleteDanhMuc = async (_id) => {
    if (window.confirm("Are you sure?")) {
      try {
        await axiosInstance.delete(`/danh-muc/delete/${_id}`);
  ...
  const filteredUsers = user.filter((item) =>
    item.ma_danh_muc.toLowerCase().includes(searchTerm.toLowerCase().trim())
  );
                ...
                filteredUsers.map((admin) => (
                  <tr key={admin._id}>
                    ...
                    <td className="py-2 pl-3 flex gap-2">
                      ...
                      <button
                        onClick={() => handleDeleteDanhMuc(admin._id)}
