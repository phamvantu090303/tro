...
export default function AccountAdmin() {
  ...
  const toggleBlock = async (admin) => {
    try {
      ...
      await axiosInstance.post(`/admin/update/${admin._id}`, {
        ...admin,
        is_block: updatedStatus,
      });
      ...
      setUser((prevUsers) =>
        prevUsers.map((u) =>
          u._id === admin._id ? { ...u, is_block: updatedStatus } : u
        )
      );
  ...
  return (
    <div className="flex h-screen gap-3">
      <div className="w-full bg-gray-100 p-6 rounded-lg shadow-lg text-black">
        ...
        {modalType && (
          <Modal
            title={modalType === "edit" ? "Edit Admin" : "Create Admin"}
            onClose={closeModal}
          >
          ...
          <table className="min-w-full bg-white text-left">
            ...
            <tbody>
              {currentData.length ? (
                    ...
                    <td className="py-2 pl-3">
                      <button
                        className={`p-2 rounded-lg ${
                          user.is_block ? "bg-red-500" : "bg-green-500"
                        } text-white`}
                ...
                <tr>
                  <td
                    colSpan={Object.keys(defaultAdmin).length + 2}
