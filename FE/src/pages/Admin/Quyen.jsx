...
const QuyenManagement = () => {
  ...
  const handleAddQuyen = async () => {
    try {
      ...
      const newQuyen = {
        ...
        trang_thai: Number(values.trang_thai),
      };

      await axios.post(
        "http://localhost:5000/phan_quyen/CreatQuyen",
        newQuyen,
        {
  ...
  const handleUpdateQuyen = async () => {
    try {
      ...
      await axios.post(
        `http://localhost:5000/phan_quyen/UpdateQuyen/${selectedQuyen._id}`,
        updatedQuyen,
        {
          headers: { "Content-Type": "application/json" },
        }
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="flex gap-2 justify-center">
          <Button type="primary" onClick={() => showFunctionList(record)}>
            Cấp Quyền
          </Button>
          <Button
            type="default"
          ...
          <Button
            ...
            danger
            icon={<DeleteOutlined />}
            onClick={() => handleDeleteQuyen(record._id)}
          />
        </div>
      ),
    },
        ...
        <div className="bg-white p-5 rounded-lg shadow-md w-full md:w-1/3">
          ...
          <div className="mb-3">
            <p className="font-semibold text-gray-700">
              Cấp quyền cho:{" "}
              <span className="text-blue-500">{selectedQuyen?.ten_quyen}</span>
            </p>
            {functionList.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {functionList.map((func) => (
                  <div
                    key={func._id}
                    className="flex items-center space-x-2 p-2 border rounded-md shadow-sm"
                  >
                    <input
                      type="checkbox"
                      id={func._id}
