...
export class SuaChuaService {
    ...
    async UpdateSuachua(id: string, data: object) {
        try {
            return await SuachuaModel.findByIdAndUpdate(id, data, { new: true, runValidators: true }).lean() || null;
        } catch (error) {
    ...
    async UpdateStatus(id: string, status: string) {
        try {
            return await SuachuaModel.findByIdAndUpdate(id, { status }, { new: true, runValidators: true }).lean() || null;
        } catch (error) {
