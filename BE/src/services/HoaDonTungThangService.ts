...
export class HoaDonThangService {
    async taoHoaDon(ma_phong: string, id_users: string, thang: string) {
        try {
            ...
            if (!dichVu) throw new Error('Không tìm thấy thông tin dịch vụ cho phòng này');
            ...
            if (soDienTieuThu < 0) {
                throw new Error('Số điện tiêu thụ không hợp lệ (âm), kiểm tra lại chỉ số điện');
            }
            ...
            const hoaDon = new HoaDonTungThangModel({
                ...
                dich_vu: dichVu._id,
                electricity_data: latestElectricity._id, // Chỉ lưu _id của bản ghi mới nhất
                tong_tien: tongTien,
            });
            ...
            throw new Error(`Lỗi khi tạo hóa đơn: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getUserHoaDon(id_user: string): Promise<any[]> {
        ...
        async updateData(_id: any, data: any): Promise<void> {
            ...
            const {ma_phong, id_users, chi_so_dien_thang_nay, chi_so_dien_thang_truoc, so_dien_tieu_thu, tien_dien, tien_phong, dich_vu, tong_tien, trang_thai, ngay_tao_hoa_don} = data;
            ...
            hoaDon.chi_so_dien_thang_nay = chi_so_dien_thang_nay ?? hoaDon.chi_so_dien_thang_nay;
            hoaDon.chi_so_dien_thang_truoc = chi_so_dien_thang_truoc ?? hoaDon.chi_so_dien_thang_truoc;
